...
class Load(mosaik_api.Simulator):
    def __init__(self):
        super().__init__(META)
        self.eid_counters = {}
    ...
    def init(self, sid, time_resolution=1.0, fmu_filename=None, instance_name=None, step_size=None, 
        ...
        self.vr_inputs = []  # Load has no inputs
        self.vr_outputs = [self.vrs['P_load_mw'], self.vrs['Q_load_mvar']]

        print(self.vrs)
        print("Initialization of FMU Load successful")
    ...
    def create(self, num, model, p_mw=0.0, q_mvar=0.0):
        ...
        for i in range(num):
            ...
            if self.model_description.fmiVersion == '2.0':
                ...
                # Set fixed load parameters during initialization
                self._entities[eid].setReal([self.vrs['p_mw']],   [p_mw])
                self._entities[eid].setReal([self.vrs['q_mvar']], [q_mvar])
                ...
                # Set fixed load parameters during initialization
                fmu.setFloat64([self.vrs['p_mw']],   [p_mw])
                fmu.setFloat64([self.vrs['q_mvar']], [q_mvar])
