...
# ── Battery CIM parameters (from IEEE_14_feeder_NEPLAN_CIM_BatteryStorage.xml) ─
_BATTERY_BUS        = 'BUS_5   69.0'   # ConnectivityNode the battery is attached to
_BATTERY_P_CHARGE   = 0.030            # Charge setpoint [MW] (positive=charge, 30 kW = CIM nominalP)

# ── PV CIM parameters (from IEEE_14_feeder_NEPLAN_CIM_PV.xml) ─────────────────
_PV_BUS             = 'BUS_9   13.8'  # ConnectivityNode the PV array is attached to
...
# ── PV: one FMU instance (PV-1, 50S×100P array) at BUS_9 ──────────────────────
pv_sim = world.start('PV',
    fmu_filename=os.path.join(FMU_DIR, 'PV_Python_fmi2.fmu'),
    instance_name='PV_MPPT',
    step_size=N_LIM)
_pv_e = pv_sim.PV.create(1)[0]
_entity_map['pv_1'] = _pv_e
print(f'  Instantiated PV-1 at {_PV_BUS} (S/T from {os.path.basename(_PV_CSV)})')
...
# Battery: P injection at BUS_5 (negative P_load_mw = discharge into grid)
...
# PV: P injection at BUS_9 (negative P_load_mw = generation into grid)
