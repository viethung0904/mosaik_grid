"""
Mosaik co-simulation scenario — NR-based network power flow (generic slack detection).
...
the initial_data in time_shifted connections, making the scenario work correctly for
multi-voltage networks (e.g. IEEE 14-bus with 69 kV, 13.8 kV and 18 kV zones).

NR mapping:
    Each mosaik timestep = one outer Gauss-Jacobi sweep with an exact NR inner solve.
    time_shifted=True on Sub→Line connections provides the one-step delay (Gauss-Jacobi).
    The SubstationNR FMU converges f(V) = I_in − conj(S/V) − jB·V = 0 exactly per tick,
    warm-started from the previous tick voltage.  N_LIM=5 outer sweeps per physical step
    mirrors the LIM scenario schedule for a direct comparison.
...
_lv_slack_subs = {_tr['lv_sub'] for _tr in _tr_edges_pre
                  if _tr['lv_sub'] not in _slack_subs
                  and not _sub_params.get(_tr['lv_sub'], {}).get('is_sync_machine', False)}
...
# With NR inner solve, convergence per sweep is much tighter than LIM, so N_LIM=5
# is retained here for a fair schedule comparison.  It can safely be reduced to 2-3
# for meshed networks once NR convergence per step is verified.
N_LIM = 5
...
# ── All substations: NR inner solver (SubstationNR FMU, step_size=1) ──
# Each tick is one outer Gauss-Jacobi iteration with an exact NR inner solve.
# The key fix: compute I_nbr = I_in + Y_self*V_old to remove the self-coupling
# injected by time-shifted line FMUs before passing to the NR solver.
# Solving f(V) = I_nbr - conj(S/V) - Y_self*V - jB*V = 0 gives an outer-iteration
# self-eigenvalue ≈ 1 - 2*omega (= 0 at omega=0.5): unconditionally stable.
...
# Lines: from-bus / to-bus voltages (time-shifted Gauss-Jacobi) + current injection
# Warm-started from CIM SvVoltage reference values; NR converges each tick exactly.
for _edge in _line_edges:
    _fs, _ts = _edge['from_sub'], _edge['to_sub']
