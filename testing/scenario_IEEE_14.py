...
# Add database/ to path so visualize can be imported
...
# I_nbr = I_in + Y_self·V_old removes self-coupling from time-shifted line FMUs.
...
# Regenerate graph.html from the currently loaded scheme in Neo4j
# so the topology visualization always matches the active simulation.
try:
...
try:
    ...
    fig.update_xaxes(title_text='Physical time (s)', row=4, col=1)
    ...
    fig.update_layout(title_text='IEEE 14-bus NR Co-simulation — 24h quasi-static', hovermode='x unified')
