"""
Mosaik co-simulation scenario — LIM power flow using the standard Transformer FMU.

This adapts the `scenario_transformer_branch_NR.py` layout but uses the normal
`Transformer` model (TR1.fmu) and `SubstationNR` for the network buses.

Notes:
 - TransformerBranch-specific current injections are removed (the FMU does not
   expose the same current ports). Y_self contributions from transformer series
   impedances are still accounted for in the bus `Y_self` values.
 - Collector mappings for transformer losses use `P1`/`Q1` and `V2` from the
   `Transformer` FMU for monitoring/comparison.
...
# Contributions from transformer series impedances (referred to HV)
for _t in _transformers:
    _r_hv, _x_hv = _tr_x_hv(_t)
...
# ── Transformer entities (standard Transformer FMU) ──────────────────────────
...
# Transformers: connect HV bus voltages (V1) to Transformer FMU (time-shifted)
for _tr in _tr_edges:
    ...
    # Monitor Transformer outputs (LV voltage and sending-end P/Q)
    _tv = _tr_var(_tr['tr_name'])
...
# Transformer monitoring (use P1/Q1 as a proxy for sending-end power/losses)
for _t in _transformers:
    _tv = _tr_var(_t['name'])
    # Map transformer sending-end power to the same P_loss keys used by the branch scenario
    world.connect(_entity_map[_tv], collector, ('P1',   f'P_loss_{_tv}_mw'))
    world.connect(_entity_map[_tv], collector, ('Q1',   f'Q_loss_{_tv}_mvar'))

# HV monitoring (optional v_source)
_vsrc = _entity_map.get('v_source')
if _vsrc is not None:
    world.connect(_vsrc, collector, ('V_source_mag', 'V_HV_mag'))
...
try:
    ...
    fig.update_layout(
        title_text='LIM Co-simulation: Transformer (standard FMU) + SubstationNR',
        hovermode='x unified',
    )
...
try:
    ...
    if not os.path.exists(_ad_path):
        ...
        fig_cmp.add_trace(go.Bar(
            name='Transformer (standard)', x=_buses, y=_v_tb_mag,
        ...
        if any(v is not None for v in _ang_tb + _ang_ad):
            fig_cmp.add_trace(go.Bar(
                name='TR angle', x=_nonslack_subs, y=_ang_tb,
        ...
        if _tr_names and any(v is not None for v in _ploss_tb + _ploss_ad):
            fig_cmp.add_trace(go.Bar(
                name='TR P_loss', x=_tr_names, y=_ploss_tb,
                marker_color='steelblue', showlegend=False, legendgroup='tb'),
                row=3, col=1)
            fig_cmp.add_trace(go.Bar(
        ...
        fig_cmp.update_layout(
            title_text='Scenario Comparison: Transformer (standard) vs Adaptive (TR1)',
            barmode='group',
            hovermode='x unified',
            height=900,
        )
